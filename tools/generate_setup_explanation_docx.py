...
OUTPUT_PATH = REPO_ROOT / "docs" / "Giai_thich_code_thiet_lap_project.docx"
...
def build_content() -> list[tuple[str, str]]:
    ...
    return [
        ...
        (
            "Normal",
            "`requirements.txt`: danh sách dependency dùng chung cho scrape dữ liệu, xử lý dữ liệu, fine-tune và phục vụ API.",
        ),
        (
            "Normal",
            "`data/`: nơi chứa dữ liệu theo từng giai đoạn của pipeline. `raw/` là dữ liệu thô, `processed/` là dữ liệu đã chunk, `training/` là dữ liệu huấn luyện cuối cùng.",
        ),
        (
            "Normal",
        ...
        (
            "Normal",
            "`notebooks/exploration.ipynb`: notebook thử nghiệm. Hiện chỉ có 1 cell markdown giới thiệu, chưa có code phân tích dữ liệu.",
        ),
        ...
        (
            "Normal",
            "Ở cấu trúc hiện tại, root `.env` là nơi lưu cấu hình dùng chung. Riêng `tools/test_api.py` có nạp `.env` bằng `python-dotenv`, còn các phần khác của project vẫn chủ yếu đọc đường dẫn hoặc cấu hình trực tiếp.",
        ),
        ...
        (
            "Normal",
            "Có hai điểm kỹ thuật cần ghi chú. Thứ nhất, `train.py` phụ thuộc `PyYAML`, vì vậy môi trường nên được cài từ `requirements.txt` mới nhất. Thứ hai, `tools/test_api.py` dùng `python-dotenv` để đọc root `.env`, nên dependency này cũng cần tồn tại trong môi trường chạy.",
        ),
        ...
        (
            "Normal",
            "Khối `if __name__ == \"__main__\":` dùng `argparse` để biến file này thành script dòng lệnh. Script nhận `url` và `output`. Ở cấu trúc mới, output mặc định trỏ về root `data/raw/document.json`.",
        ),
        (
            "Normal",
        ...
        (
            "Normal",
            "Một chi tiết vận hành quan trọng là `train.py` hiện đọc `config.yaml` theo vị trí file script, nên việc chạy từ root repo ổn định hơn so với bản cấu trúc cũ dùng đường dẫn tương đối theo thư mục hiện tại.",
        ),
        (
            "Normal",
        ...
        (
            "Normal",
            "5. Cần giữ `requirements.txt` đồng bộ với code để các dependency như `PyYAML` và `python-dotenv` luôn có sẵn khi dựng môi trường mới.",
        ),
        (
            "Normal",
